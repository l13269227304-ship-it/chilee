...
export default function SignupPage() {
  ...
  if (done) return (
    <div className="min-h-full flex flex-col items-center justify-center px-6" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm bg-white rounded-2xl p-6 text-center shadow-sm" style={{ border: '0.5px solid var(--sep)' }}>
        <div className="text-3xl mb-3">📬</div>
        <h2 className="text-base font-semibold mb-2" style={{ color: 'var(--text1)' }}>验证邮件已发送</h2>
        <p className="text-sm" style={{ color: 'var(--text2)' }}>请查收 {email} 的邮件，点击链接完成注册</p>
        <Link href="/login" className="block mt-4 text-sm font-medium" style={{ color: 'var(--blue)' }}>返回登录</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-12" style={{ background: 'var(--bg)' }}>
      <div className="mb-8 text-center">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text3)' }}>注册后数据永久保存在云端</p>
      </div>

      <div className="w-full max-w-sm bg-white rounded-2xl p-6 shadow-sm" style={{ border: '0.5px solid var(--sep)' }}>
        <h2 className="text-lg font-semibold mb-5" style={{ color: 'var(--text1)' }}>创建账号</h2>
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text2)' }}>密码（至少6位）</label>
            <input
          ...
          <button
            ...
            className="w-full h-11 rounded-lg text-white text-sm font-semibold disabled:opacity-40"
        ...
        <p className="text-center text-xs mt-4" style={{ color: 'var(--text3)' }}>
          已有账号？{' '}
          <Link href="/login" className="font-medium" style={{ color: 'var(--blue)' }}>登录</Link>
        </p>
      </div>
    </div>
  )
